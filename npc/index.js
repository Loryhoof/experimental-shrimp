...
export default e => {
  ...
  if (mode === 'attached') {
    ...
    let avArray = ["meebit.vrm", "rabbit.vrm"];
    let pickedAvatar = avArray[Math.floor(Math.random() * avArray.length)];
    const npcAvatarUrl = app.getComponent('avatarUrl') ?? `/scenes/npc/` + pickedAvatar.toString();
    ...
    const runSpeed = walkSpeed * (Math.random() * (5 - 2) + 2);
    ...
    useFrame(({timestamp, timeDiff}) => {
      if (npcPlayer && physics.getPhysicsEnabled()) {
        ...
        if (npcPlayer.hasAction('go')) {
          ...
          targetSpec = {
            type: 'moveto',
            object: new THREE.Vector3(250, -9, Math.floor(Math.random() * 60) + -60),
          };
          npcPlayer.removeAction('go');
        }
        if (targetSpec) {
          const target = targetSpec.object;
          const v = target.sub(npcPlayer.position);
          v.y = 0;
